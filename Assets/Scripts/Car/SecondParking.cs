...
public class SecondParking : MonoBehaviour
{
    ...
    public bool HasFreePlace()
    {
        ...
        //return _carPlacesInQueue.Any(place => place.IsFree && !place.IsLocked);
    }

    public bool TakePlace(CarWithSeats carWithSeats)
    {
        CarPlace freePlace = _carPlacesQueue.GetFreePlace();
        //CarPlace freePlace = _carPlacesInQueue.FirstOrDefault(place => place.IsFree && !place.IsLocked);
    ...
    {
        return _carsInQueue.FirstOrDefault(car => car.State == CarWithSeatsState.Parked && car.HasFreeSeats && car.ColorIndex == colorIndex);
    }

    private void ParkedCar(CarWithSeats car)
    {
        //_carsInQueue.Add(car);
